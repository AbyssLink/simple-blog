...
function prepareForMobile(){//移动设备的css样式
	...
	$("#grid-container").css('border-radius',0.02*gridContainerWidth);//圆角
	...
	for(var i=0;i<5;i++)
		for(var j=0;j<5;j++){
			...
			if (board[i][j] == 0){
				...
				theNumberCell.css('background-color',getNumberBackgroundColor(board[i][j]));//不同数字，不同的背景颜色
		...
		$('.number-cell').css('font-size',0.6*cellSlideLength + 'px');//设置字体大小
}
/**
 * 随机生产数字2，4
 */
